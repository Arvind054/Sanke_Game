...
let food = { x: 8, y: 10 };
let score = 0;
let currentScoreElement = document.getElementsByClassName("currentScore")[0];
let highScoreElement = document.getElementsByClassName("HighestScore")[0];
let highScore = localStorage.getItem("highScore");
if (highScore === null) {
  localStorage.setItem("highScore", JSON.stringify(0));
} else {
  let highScoreVal = JSON.parse(highScore);
  highScoreElement.innerHTML = `Highest: ${highScoreVal}`;
}
//game functions
